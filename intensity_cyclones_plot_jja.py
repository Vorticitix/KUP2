...
for rank in ranks:
    ...
    if compare_med==False:
        stat_extra_emed = ttest_ind(xr.open_dataset(path+file.format('emed_JJA',events[0],rank)).isel(time=8)['WS'].values,
                                    xr.open_dataset(path+file.format('emed_JJA',events[1],rank)).isel(time=8)['WS'].values,
                                    equal_var=False)[1]
        stat_extra_cmed = ttest_ind(xr.open_dataset(path+file.format('cmed_JJA',events[0],rank)).isel(time=8)['WS'].values,
                                    xr.open_dataset(path+file.format('cmed_JJA',events[1],rank)).isel(time=8)['WS'].values,
                                    equal_var=False)[1]
    for i in range(2):
        for j in range(3):
            ds_cmed = xr.open_dataset(path+file.format('cmed_JJA',events[j],rank)).isel(time=8)
            ds_emed = xr.open_dataset(path+file.format('emed_JJA',events[j],rank)).isel(time=8)
            stat_WS = ttest_ind(ds_cmed['WS'].values,ds_emed['WS'].values,equal_var=False)[1]
            if i == 0:
    ...
    fig.tight_layout()
    if compare_med==True:
        fig.savefig(path+'figs/rank_{}_spatial_WS_comparison_JJA{}.png'.format(rank,maxstring),dpi=400)
    elif compare_med==False:
        fig.savefig(path+'figs/rank_{}_spatial_WS_comparison_reg_JJA{}.png'.format(rank,maxstring),dpi=400)
...
for rank in ranks:
    ...
    fig = obj.fig; axz = axz = obj.axz
    if compare_med==False:
        stat_extra_emed = ttest_ind(xr.open_dataset(path+file.format('emed_JJA',events[0],rank)).isel(time=8)['PRECT'].values,
                                    xr.open_dataset(path+file.format('emed_JJA',events[1],rank)).isel(time=8)['PRECT'].values,
                                    equal_var=False)[1]
        stat_extra_cmed = ttest_ind(xr.open_dataset(path+file.format('cmed_JJA',events[0],rank)).isel(time=8)['PRECT'].values,
                                    xr.open_dataset(path+file.format('cmed_JJA',events[1],rank)).isel(time=8)['PRECT'].values,
                                    equal_var=False)[1]
    for i in range(2):
        for j in range(3):
            ds_cmed = xr.open_dataset(path+file.format('cmed_JJA',events[j],rank)).isel(time=8)
    ...
    fig.tight_layout()
    if compare_med==True:
        fig.savefig(path+'figs/rank_{}_spatial_PRECT_comparison_JJA{}.png'.format(rank,maxstring),dpi=400)
    else:
        fig.savefig(path+'figs/rank_{}_spatial_PRECT_comparison_reg_JJA{}.png'.format(rank,maxstring),dpi=400)
...
for i in range(nrows):
    for j in range(ncols):
        ax = axz[i,j]
        if compare_med==False:
            ds_cmed_10 = xr.open_dataset(path+file.format('cmed_JJA',events[j],10))
            ds_cmed_100 = xr.open_dataset(path+file.format('cmed_JJA',events[j],100))
            ds_cmed_1000 = xr.open_dataset(path+file.format('cmed_JJA',events[j],1000))
            ds_emed_10 = xr.open_dataset(path+file.format('emed_JJA',events[j],10))
            ds_emed_100 = xr.open_dataset(path+file.format('emed_JJA',events[j],100))
            ds_emed_1000 = xr.open_dataset(path+file.format('emed_JJA',events[j],1000))
            if vars[i]=='PSL':
        ...
        for rank in ranks:
            ds_cmed = xr.open_dataset(path+file.format('cmed_JJA',events[j],rank))
            ds_emed = xr.open_dataset(path+file.format('emed_JJA',events[j],rank))
        ...
        ax.set_xticks(np.arange(-48,48.1,6))
...
sys.exit()
if compare_med==True:
    fig.savefig(path+'figs/surface_evolution_compare_med_JJA.png',dpi=400)
else:
    fig.savefig(path+'figs/surface_evolution_compare_number_JJA.png',dpi=400)
#%%
path = '/storage/climatestor/PleioCEP/doensen/data/cyclone_intensity/'
file = '{}_{}_top_{:04d}_all.nc'
...
for rank in ranks:
    ...
    fig.tight_layout()
    if compare_med==True:
        fig.savefig(path+'figs/rank_{}_spatial_V300_T850_comparison.png'.format(rank),dpi=400)
    else:
        fig.savefig(path+'figs/rank_{}_spatial_V300_T850_comparison_reg.png'.format(rank),dpi=400)
...
for rank in ranks:
    ...
    fig.tight_layout()
    if compare_med==True:
        fig.savefig(path+'figs/rank_{}_spatial_RWP_Z500_comparison.png'.format(rank),dpi=400)
    else:
        fig.savefig(path+'figs/rank_{}_spatial_RWP_Z500_comparison_reg.png'.format(rank),dpi=400)
...
for i in range(nrows):
    for j in range(ncols):
        ax = axz[i,j]
        if compare_med==False:
            ds_cmed_10 = xr.open_dataset(path+file.format('cmed',events[j],10))
            ds_cmed_100 = xr.open_dataset(path+file.format('cmed',events[j],100))
            ds_cmed_1000 = xr.open_dataset(path+file.format('cmed',events[j],1000))
            ds_emed_10 = xr.open_dataset(path+file.format('emed',events[j],10))
            ds_emed_100 = xr.open_dataset(path+file.format('emed',events[j],100))
            ds_emed_1000 = xr.open_dataset(path+file.format('emed',events[j],1000))
        ...
        for rank in ranks:
            ds_cmed = xr.open_dataset(path+file.format('cmed',events[j],rank))
            ds_emed = xr.open_dataset(path+file.format('emed',events[j],rank))
            ds_cmed_mean = ds_cmed.mean(dim='rank')
            ds_emed_mean = ds_emed.mean(dim='rank')
...
fig.tight_layout()
if compare_med==True:
    fig.savefig(path+'figs/upperatm_evolution_compare_med.png',dpi=400)
else:
    fig.savefig(path+'figs/upperatm_evolution_compare_number.png',dpi=400)
...
for region in regions:
    for event in events:
        ...
        fig.savefig(path+'figs/{}_{}_latlon.png'.format(region,event),dpi=400)
