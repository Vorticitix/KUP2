...
file_med = 'fort_36_total_eur_full.txt'
...
T850 = xr.open_dataset(path+'extracted/T850/T850_0005_3601_anom_eur_mean.nc').squeeze()
...
for i,area in enumerate(areas):
    ...
    for j,quant in enumerate(quants):
        ...
        fig.savefig(path+'figs/Timeseries_future_99p_eur.png',dpi=300)
        sys.exit()
...
def cov_plot(df):
    ...
    for y in range(len(columns)):
        for x in range(len(columns)):
            ...
            ax.label_outer()
    fig.tight_layout()
    fig.savefig(path+'cov_plot_past_med.png',dpi=300)
    return fig
