...
temp_loc = 'natl'
T850 = xr.open_dataset(path+'extracted/T850/T850_0005_3601_anom_{}_mean.nc'.format(temp_loc)).squeeze()
...
T850 = T850.T.rolling(time=90).mean()
...
for i,area in enumerate(areas):
    path = "/storage/climatestor/PleioCEP/doensen/data/"
    # file_past = 'fort_36_total_med.txt'
    file = 'fort_36_total_{}_full.txt'.format(area)
    ...
    for j,quant in enumerate(quants):
        ...
        fig.savefig(path+'figs/Timeseries_future_99p_track_{}_temp_{}.png'.format(track_loc,temp_loc),dpi=300)
        sys.exit()
...
import matplotlib
df_cov = pd.DataFrame({"T850 Anomaly [K]": T850.values.squeeze(),
...
def cov_plot(df):
    ...
    for y in range(len(columns)):
        for x in range(len(columns)):
            ...
            ax.label_outer()
    fig.tight_layout()
    fig.savefig(path+'cov_plot_past_track_{}_temp_{}.png'.format(track_loc,temp_loc),dpi=300)
    return fig
