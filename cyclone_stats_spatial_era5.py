...
file = "fort_36_total_whem_ERA5.txt"

#Open cyclone tracks from CESM
df = pd.read_csv(path+file,header=0,index_col=0)
df = df.where((df['year']>=2010)&(df['year']<=2010)).dropna()
...
for i in length:
    ...
    if (i==0)|(np.isin(i,jump_years)):
        if i!=0:
            ds.to_netcdf(path+'cyclone_spatial_stats/ERA5_{:04d}_XL.nc'\
...
#        assert(total[i,3]>=total[i+1,3])
        file_framework = 'ERA5_{:04d}_merge2.nc'.format(int(year))
        ds = xr.open_dataset(path+'cyclone_era5_1981_2010/'+file_framework)
        ds.coords['lon'] = (ds.coords['lon'] + 180) % 360 - 180
        ds = ds.sortby(ds.lon).squeeze()
        ds = ds.rename(name_dict={'msl':'PSL'})
        newtime = [ cftime.DatetimeNoLeap(year=int(x.dt.year),month=int(x.dt.month),day=int(x.dt.day),\
                    hour=int(x.dt.hour),has_year_zero=True) for x in ds.time]
        ds['time'] = newtime
    ...
    ds['CZ'].values[time_idx,dist] = ds_raw['z'].values[time_idx,dist]
    ds['ZRAD'].values[time_idx,dist] = zrad
    ds['ZDEP'].values[time_idx,dist] = zdep
    ds['PRECMEAN'].values[time_idx,dist] = ds_raw['prec'].values[time_idx,dist]
