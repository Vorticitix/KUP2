...
for temp_loc in regions:
    ...
    T850_RCP85 = T850_RCP85.where(T850_RCP85.time.dt.year>=3514)
    ...
    fig,axz=plt.subplots(4,1,figsize=(7.5,6))
    ...
    axz[0].set_ylabel('T850 Anomaly [K]',fontsize=8)
    axz[0].set_ylim([-.5,.5])
    ...
    axz[1].set_ylabel('Total Cyclone \nTime Steps [$\mathregular{month^{-1}}$]',fontsize=8)
    ...
    axz[2].set_ylabel('Precipitation [mm]',fontsize=8)
    axz[2].axhline(y=prec_mean,color='k')
    axz[2].axvline(x=2005,color='g')   
    #axz[4].set_ylim([18,21.5])
    axz[2].fill_between(index, prec_mean, prec.values.squeeze(), where=prec.values.squeeze() >= prec_mean, facecolor='red', interpolate=True)
    axz[2].fill_between(index, prec_mean, prec.values.squeeze(), where=prec.values.squeeze() <= prec_mean, facecolor='blue', interpolate=True)
    #axz[5].plot(index,ws.values)
    axz[3].set_ylabel('Wind Speed [m/s]',fontsize=8)
    axz[3].axhline(y=ws_mean,color='k')
    axz[3].axvline(x=2005,color='g')   
    axz[3].fill_between(index, ws_mean, ws.values.squeeze(), where=ws.values.squeeze() >= ws_mean, facecolor='red', interpolate=True)
    axz[3].fill_between(index, ws_mean, ws.values.squeeze(), where=ws.values.squeeze() <= ws_mean, facecolor='blue', interpolate=True)
    ...
    fig.subplots_adjust(left=.12,right=.96,top=.9,bottom=.045,hspace=.075)
    fig.suptitle('Temperature Anomalies and 1% most Extreme Cyclone features\n RCP8.5 Scenario for the Mediterranean DJF')
    ...
    fig.savefig(path+'figs/Timeseries_past_EGU.png'.format(track_loc,temp_loc,period),dpi=300)
    #plt.close(fig)
