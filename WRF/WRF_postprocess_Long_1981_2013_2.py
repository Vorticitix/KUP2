...
for i,file in enumerate(files):
    ...
    my_file = Path(path_to_save+'Long_1981_2013/'+exist_file)
    if my_file.is_file():
        print(exist_file+' exists')
        continue
    if int(exist_date[:4])<2001:
        print(exist_date)
        continue
    elif int(exist_date[:4])>2010:
        sys.exit()
    try:
        wrfout = Dataset(file)
    except:
    ...
    #Path(path_to_save+'Long_1981_2013/prec/').mkdir(parents=True,exist_ok=True)
    #Define date for file to save. Then save to netcdf file
    init_time = prec.Time[0].dt.strftime("%Y-%m-%d")
    print(int(str(init_time.values)[:4]))
    if int(str(init_time.values)[:4])<2001:
        continue
    elif int(str(init_time.values)[:4])>2010:
        sys.exit()
    ...
    #ds.to_netcdf(path_to_save+'Long_1981_2013/prec/prec_{}.nc'.format(str(init_time.values)))
    ...
    for flux in fluxs:
        #Path(path_to_save+'Long_1981_2013/{}/'.format(flux)).mkdir(parents=True,exist_ok=True)
        ...
        total_2D.append(ds)
        #ds.to_netcdf(path_to_save+'Long_1981_2013/{}/{}_{}.nc'.format(flux,flux,str(init_time.values)))
    ...
    ds_2d.to_netcdf(path_to_save+'Long_1981_2013/WRF_{}_2D.nc'.format(str(init_time.values)))
    ...
    for key in vars_3D:
        ...
        ds = da.to_dataset(name=key)
        init_time = da.Time[0].dt.strftime("%Y-%m-%d")
        #Path(path_to_save+'Long_1981_2013/{}/'.format(key)).mkdir(parents=True,exist_ok=True)
        total_3D.append(ds)
        #ds.to_netcdf(path_to_save+'Long_1981_2013/{}/{}_{}.nc'.format(key,key,str(init_time.values)))
    ds_3d = xr.merge(total_3D)
    ds_3d.to_netcdf(path_to_save+'Long_1981_2013/WRF_{}_3D.nc'.format(str(init_time.values)))
