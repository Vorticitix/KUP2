...
for i,file in enumerate(files):
    ...
    my_file = Path(path_to_save+'Long_RCP85_2037_2069/'+exist_file)
    if my_file.is_file():
    ...
    #Path(path_to_save+'Long_RCP85_2037_2069/prec/').mkdir(parents=True,exist_ok=True)
    ...
    #ds.to_netcdf(path_to_save+'Long_RCP85_2037_2069/prec/prec_{}.nc'.format(str(init_time.values)))
    ...
    for flux in fluxs:
        #Path(path_to_save+'Long_RCP85_2037_2069/{}/'.format(flux)).mkdir(parents=True,exist_ok=True)
        ...
        total_2D.append(ds)
        #ds.to_netcdf(path_to_save+'Long_RCP85_2037_2069/{}/{}_{}.nc'.format(flux,flux,str(init_time.values)))
    ...
    ds_2d.to_netcdf(path_to_save+'Long_RCP85_2037_2069/WRF_{}_2D.nc'.format(str(init_time.values)))
    ...
    for key in vars_3D:
        ...
        ds = da.to_dataset(name=key)
        init_time = da.Time[0].dt.strftime("%Y-%m-%d")
        #Path(path_to_save+'Long_RCP85_2037_2069/{}/'.format(key)).mkdir(parents=True,exist_ok=True)
        total_3D.append(ds)
        #ds.to_netcdf(path_to_save+'Long_RCP85_2037_2069/{}/{}_{}.nc'.format(key,key,str(init_time.values)))
    ds_3d = xr.merge(total_3D)
    ds_3d.to_netcdf(path_to_save+'Long_RCP85_2037_2069/WRF_{}_3D.nc'.format(str(init_time.values)))
