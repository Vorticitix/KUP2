...
#ax.coastlines()   
ani = animation.FuncAnimation(fig,animate,frames=len(cf_dates),interval=2000)
#fig.colorbar(cont)
...
ax.set_extent([-15, 60, 25, 60])
ax.coastlines()
PSL = slp.sel(Time=slice(np.datetime64('1990-01-17T06:00:00.000000000'),
                        np.datetime64('1990-01-19T11:00:00.000000000'))).slp[:,:,0]
cont = ax.contour(lon,lat,PSL,transform=ccrs.PlateCarree(),
                vmin=97000,vmax=105000,cmap="binary")
# im = ax.contourf(lon,lat,ws,transform=ccrs.PlateCarree(),
#                 vmin=0,vmax=30,cmap="hot_r")
...
def animate(i):
    global cont, im, scat
    ws = ws850.sel(Time=slice(np.datetime64('1961-01-17T06:00:00.000000000'),
                            np.datetime64('1961-01-19T11:00:00.000000000'))).ws
    PSL = slp.sel(Time=slice(np.datetime64('1961-01-17T06:00:00.000000000'),
                            np.datetime64('1961-01-19T11:00:00.000000000'))).ws
    ax.clear()
    ax.coastlines(zorder=3)
    ax.set_title('{:02d}-{:02d}-{:04d} {:02d}:00     AGE = {}'.format(cf_dates[i].day,
                                        ws850.Time.dt.month,
                                        ws850.Time.dt.month.year,
                                        ws850.Time.dt.month.hour,
...
#ax.coastlines()   
ani = animation.FuncAnimation(fig,animate,frames=len(PSL.Time),interval=400)
#fig.colorbar(cont)
