...
mkdir /storage/climatestor/PleioCEP/doensen/data/cyclone_0005_0604

cd /storage/climatestor/PleioCEP/doensen/data/cyclone_0005_0604
...
!      integer agenew(3000,10000,3)
	...
	Open (34,File="PRECL.srv",FORM='UNFORMATTED',ACCESS='SEQUENTIAL',status='OLD')
      Open (35,File="PRECC.srv",FORM='UNFORMATTED',ACCESS='SEQUENTIAL',status='OLD')
...
!	open (unit=327,file='ws.txt',STATUS='REPLACE')

!      do ff=1,96
!    write(327,'(*(F20.7,:,","))') ws(:,ff)
!	end do
!	close(327)
!	stop
	  k=k+1
...
!            print*,k,date,islp(k)
 ...
 !           if (prec(i,j).lt.0.) print*,i,j,k,prec(i,j), rprec(i,j,k) 
	...
	call varstats(precl,ilon,ilat,zrad,preclmmean,preclmmax)
      call varstats(precc,ilon,ilat,zrad,preccmmean,preccmmax)
	...
	print*,precmmean,precmmax,preccmmean,preclmmean,wsmmean,wsmmax
      write(36,777) iic, t0, age, ilon, ilat, cat, date,cz, di, realc, gz, &
			  ...
			  precmmax,preccmmean,preclmmean,wsmmean,wsmmax
      print*,ilon, ilat,k,rslp(ilon, ilat,k)/100.
      write(37,*)cz
      write(38,*)rslp(ilon, ilat,k)
	  write(39,*)wsmmean
	  write(40,*)precmmean
	  write(41,*)preclmmean
	  write(42,*)preccmmean 
      write(43,*)date
      write(44,*)wsmean_test
      write(45,*)precmean_test

!	count = count + 1
!	if (count.eq.50) then
!	end if
777   format(2i12,i4,2i4,i2,i9,e13.4,f7.3,i2,e13.4,i4,i7,f8.3,8e13.4) 
      goto 800
...
998   continue
      ...
      subroutine varstats(svar,ilon, ilat, zrad, varmmean, varmmax)
      implicit none
      integer i,j,k,ilon, ilat,l,m, icount
      integer ni, nj,nloout, nlaout
      real rlon,rlat,pi
      real svar(144,96), zrad, varmean,varmmean
...
!                 print*,latxy,lonxy
                 ...
                 if (radxy .lt. zrad) then
                    ...
                    varmmean =varmmean + svar(l, m) * sin(pi*(m)/nlaout)                    
                    if (varmmax .lt. svar(l,m)*sin(pi*(m)/nlaout)) then
				varmmax = svar(l,m)*sin(pi*(m)/nlaout)
			  end if
			   icount = icount +1
                 end if
             end do 
...
/bin/mv fort.39 fort_39_${arr[1]}"_"${arr[10]}
/bin/mv fort.44 fort_44_${arr[1]}"_"${arr[10]}
/bin/mv fort.45 fort_45_${arr[1]}"_"${arr[10]}

#/bin/mv fort.40 fort_39_${arr[1]}"_"${arr[10]}
#/bin/mv fort.41 fort_39_${arr[1]}"_"${arr[10]}
#/bin/mv fort.42 fort_39_${arr[1]}"_"${arr[10]}
