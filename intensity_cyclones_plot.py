...
for rank in ranks:
    ...
    if compare_med==False:
        stat_extra_emed = ttest_ind(xr.open_dataset(path+file.format('emed',events[0],rank)).isel(time=8)['WS'].values,
                                    xr.open_dataset(path+file.format('emed',events[1],rank)).isel(time=8)['WS'].values,
                                    equal_var=False)[1]
        stat_extra_cmed = ttest_ind(xr.open_dataset(path+file.format('cmed',events[0],rank)).isel(time=8)['WS'].values,
                                    xr.open_dataset(path+file.format('cmed',events[1],rank)).isel(time=8)['WS'].values,
                                    equal_var=False)[1]
    letter_index = 0
    for i in range(2):
        for j in range(3):
    ...
    fig.tight_layout()
    if compare_med==True:
        fig.savefig(path+'figs/rank_{}_spatial_WS_comparison{}.png'.format(rank,maxstring),dpi=400)
    elif compare_med==False:
        fig.savefig(path+'figs/rank_{}_spatial_WS_comparison_reg{}.png'.format(rank,maxstring),dpi=400)
