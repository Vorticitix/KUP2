...
#locs = ['med','eur','natl','satl']
locs = ['med','natl','satl','eur']
period = 'past'
...
for track_loc in locs:
    ...
    pcs = []
    for i in range(6):
        pc = xr.open_dataset(path+'extracted/EOF_season/PC_mode_{:02d}.nc'.format(i+1))  
        time_arr = pc.time.values
        time = np.around(np.array([x.year + x.month/12-1/12 for x in time_arr]),3)
        pc = pc.assign_coords(time=time)
        _, index = np.unique(pc.time, return_index=True)
        pc = pc.isel(time=index)
        pc = pc.reindex(time=full_time)
        pc = pc.rolling(time=90,min_periods=85).mean(dim='time',skipna=True)
        pcs.append(pc)
    dic = dict.fromkeys([0, 1, 2, 3, 4, 5])
    ...
    def cov_plot(df):
        ...
        for y in range(len(columns)):
            for x in range(len(columns)):
                ...
                ax.label_outer()
        fig.suptitle('{}'.format(track_loc))
        fig.tight_layout()
        fig.savefig(path+'figs/cov_plot_past_track_{}_modes.png'.format(track_loc),dpi=300)
