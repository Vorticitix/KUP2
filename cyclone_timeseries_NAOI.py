...
file_rcp = 'fort_36_total_{}_RCP85.txt'.format(track_loc)
...
df_rcp = df_rcp.where(df_rcp['year']>=3507).dropna(how='all')
df = df.where((df['month']>=12) | (df['month']<=2)).dropna(how='all')
df_rcp = df_rcp.where((df_rcp['month']>=12) | (df_rcp['month']<=2)).dropna(how='all')
#df['yearseason']=(df['year'].astype(int)-1502).astype(str)+'_'+df['month'].map(seasons_dic)
df['yearmonth']=df['year'] + df['month']/12 -1/12 
df_rcp['yearmonth']=df_rcp['year'] + df_rcp['month']/12 -1/12 
#df['cen'] = (df['year']-1502)//100



#df_groupby= df.groupby('yearseason')
df_gb_max = df[['zrad','zdep','precmean','preccmean','preclmean','wsmean','iic']].groupby('iic',dropna=False).max()
df_gb_min = df[['slp','iic','agetot']].groupby('iic',dropna=False).min()
df_gb_yas = df[['yearmonth','iic']].groupby('iic',dropna=False).first()
...
df_rcp_gb_max = df_rcp[['zrad','zdep','precmean','preccmean','preclmean','wsmean','iic']].groupby('iic',dropna=False).max()
df_rcp_gb_min = df_rcp[['slp','iic','agetot']].groupby('iic',dropna=False).min()
df_rcp_gb_yas = df_rcp[['yearmonth','iic']].groupby('iic',dropna=False).first()
df_rcp_gb_iic = pd.concat([df_rcp_gb_max,df_rcp_gb_min,df_rcp_gb_yas],axis=1)
df_rcp_gb_iic = df_rcp_gb_iic.where((df_rcp_gb_iic['slp']!=0)
                            &(df_rcp_gb_iic['zrad']!=0)
                            &(df_rcp_gb_iic['zdep']!=0)).dropna(how='all')


period='past'
agetot = df_gb_iic[['agetot','yearmonth']].groupby('yearmonth',dropna=False).sum()
agetot_rcp = df_rcp_gb_iic[['agetot','yearmonth']].groupby('yearmonth',dropna=False).sum()
agetot = pd.concat([agetot, agetot_rcp]).groupby(level=0).mean().rolling(90).mean()
agetot_mean = agetot[agetot.index<=3352].dropna().mean().values[0]


radius = df_gb_iic[['zrad','yearmonth']].groupby('yearmonth',dropna=False).quantile(.99)
radius_rcp = df_rcp_gb_iic[['zrad','yearmonth']].groupby('yearmonth',dropna=False).quantile(.99)
radius = pd.concat([radius, radius_rcp]).groupby(level=0).mean().rolling(90).mean()
...
slp = df_gb_iic[['slp','yearmonth']].groupby('yearmonth',dropna=False).quantile(1-.99)
slp_rcp = df_rcp_gb_iic[['slp','yearmonth']].groupby('yearmonth',dropna=False).quantile(1-.99)
slp = pd.concat([slp, slp_rcp]).groupby(level=0).mean().rolling(90).mean()
...
depth = df_gb_iic[['zdep','yearmonth']].groupby('yearmonth',dropna=False).quantile(.99)
depth_rcp = df_rcp_gb_iic[['zdep','yearmonth']].groupby('yearmonth',dropna=False).quantile(.99)
depth = pd.concat([depth, depth_rcp]).groupby(level=0).mean().rolling(90).mean()
depth_mean = depth[depth.index<=3352].dropna().mean().values[0]

# fig,ax = plt.subplots()
# ax.plot(depth.index-1502,depth.values)

prec = df_gb_iic[['precmean','yearmonth']].groupby('yearmonth',dropna=False).quantile(.99)
prec_rcp = df_rcp_gb_iic[['precmean','yearmonth']].groupby('yearmonth',dropna=False).quantile(.99)
prec = pd.concat([prec, prec_rcp]).groupby(level=0).mean().rolling(90).mean()
...
# ax.grid()
ws = df_gb_iic[['wsmean','yearmonth']].groupby('yearmonth',dropna=False).quantile(.99)
ws_rcp = df_rcp_gb_iic[['wsmean','yearmonth']].groupby('yearmonth',dropna=False).quantile(.99)
ws = pd.concat([ws, ws_rcp]).groupby(level=0).mean().rolling(90).mean()
...
NAOI = xr.open_dataset(path+'extracted/try_PSL/NAOI.nc').squeeze()
NAOI_RCP85 = xr.open_dataset(path+'extracted/try_PSL/NAOI_RCP85.nc').squeeze()
NAOI = NAOI.rename({'PSL':"NAOI"})
NAOI = NAOI.where(np.isin(NAOI.time.dt.month,[12,1,2]))
NAOI_RCP85 = NAOI_RCP85.rename({'PSL':"NAOI"})
NAOI_RCP85 = NAOI_RCP85.where(np.isin(NAOI_RCP85.time.dt.month,[12,1,2]))
tmp = xr.concat([NAOI,NAOI_RCP85],dim='time')
NAOI = tmp.groupby(tmp.time).mean()
...
# T850 = T850.assign_coords(lon=(((T850.lon.values + 180) % 360) - 180)).sortby('lon')
#time_arr = T850.time.valuesagetot_rcp[agetot_rcp.index<3515]
#time = np.array([x.year + x.month/12-1/12 for x in time_arr])
#axz[0].plot(index,T850.values)
axz[0].set_ylabel('NAOI [-]')
#axz[0].set_ylim([-.5,.5])
axz[0].fill_between(index, 0, NAOI.values, where=NAOI.values >= 0, facecolor='red', interpolate=True)
axz[0].fill_between(index, 0, NAOI.values, where=NAOI.values <= 0, facecolor='blue', interpolate=True)
...
fig.suptitle('NAOI activity for cyclones in {}'.format(track_loc))
...
fig.savefig(path+'figs/Timeseries_future_99p_track_{}_NAOI_{}.png'.format(track_loc,period),dpi=300)
#plt.close(fig)
...
import matplotlib
df_cov = pd.DataFrame({"NAOI [-]": NAOI.values.squeeze(),
...
def cov_plot(df):
    ...
    for y in range(len(columns)):
        for x in range(len(columns)):
            ...
            ax.label_outer()
    fig.tight_layout()
    fig.savefig(path+'figs/cov_plot_past_track_{}_NAOI_{}.png'.format(track_loc,period),dpi=300)
    return fig
