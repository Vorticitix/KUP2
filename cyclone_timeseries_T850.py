...
for temp_loc in regions:
    full_time = radius.index
    T850 = xr.open_dataset(path+'extracted/T850/T850_0005_3514_anom_{}.nc'.format(temp_loc)).squeeze()
    ...
    time_arr = T850_RCP85.time.values
    time = np.array([x.year + x.month/12-1/12 for x in time_arr])
    T850_RCP85 = T850_RCP85.assign_coords(time=time)
    T850_RCP85 = T850_RCP85.reindex(time=full_time)
    
    tmp = xr.concat([T850,T850_RCP85],dim='time')
    T850 = tmp.groupby(tmp.time).mean()
    ...
    #axz[0].set_ylim([-.5,.5])
    axz[0].fill_between(index, 0, T850.values, where=T850.values >= 0, facecolor='red', interpolate=True)
    axz[0].fill_between(index, 0, T850.values, where=T850.values <= 0, facecolor='blue', interpolate=True)
