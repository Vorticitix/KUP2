...
#Create a unique zeiger for every range of years
...
#Create a combined netcdf files spanning 10 years. 1378 and 1583 are corrupt so they can't be used (that's where the if statements come from)
...
#Copy relevant fields to srv file
...
#move fort 30 file to specific filename including year
...
!            print*,k,date,islp(k)
 ...
 !           if (prec(i,j).lt.0.) print*,i,j,k,prec(i,j), rprec(i,j,k) 
	...
	call varstats(precl,ilon,ilat,zrad,preclmmean,preclmmax)
      call varstats(precc,ilon,ilat,zrad,preclmmean,preclmmax)
