...
for track_loc in locs:
    ...
    file_med = 'fort_36_total_{}.txt'.format(track_loc)
    ...
    agetot = agetot.groupby('yearmonth').sum().reindex(full_time).rolling(360,min_periods=120).mean()
    ...
    radius=radius.groupby('yearmonth').quantile(.90).reindex(full_time).rolling(360,min_periods=120).mean()
    ...
    slp = slp.groupby('yearmonth').quantile(1-.90).reindex(full_time).rolling(360,min_periods=120).mean()
    ...
    depth = depth.groupby('yearmonth').quantile(.90).reindex(full_time).rolling(360,min_periods=120).mean()
    ...
    prec = prec.groupby('yearmonth').quantile(.90).reindex(full_time).rolling(360,min_periods=120).mean()
    ...
    ws = ws.groupby('yearmonth').quantile(.90).reindex(full_time).rolling(360,min_periods=120).mean()
    ...
    T850 = T850.T.rolling(time=360,min_periods=340).mean(dim='time',skipna=True)
    ...
    T850_sub = T850.sel(time=slice(5,3352)).dropna(dim='time')
    ...
    RWP = RWP.V.rolling(time=360,min_periods=340).mean(dim='time',skipna=True)
    RWP_mean = float(RWP.sel(time=slice(5,3352)).mean())
    RWP_std = float(RWP.sel(time=slice(5,3352)).std())
    ...
    NAOI = NAOI.NAOI.rolling(time=360,min_periods=340).mean(dim='time',skipna=True)
    NAOI_mean = float(NAOI.sel(time=slice(5,3352)).mean())
    NAOI_std = float(NAOI.sel(time=slice(5,3352)).std())
    ...
    sys.exit()
    fig,axz=plt.subplots(4,1,figsize=(15,8))
    ...
    axz[0].set_ylabel('T850 Anomalies [K]',fontsize=12)
    if period=='past':
        axz[0].set_ylim([-.5,.5])
    ...
    axz[0].fill_between(index, 0, T850_plus_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    axz[0].fill_between(index, 0, T850_min_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    # axz[1].fill_between(index, RWP_mean, RWP_plus_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    # axz[1].fill_between(index, RWP_mean, RWP_min_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    # axz[2].fill_between(index, 0, NAOI_plus_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    # axz[2].fill_between(index, 0, NAOI_min_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    axz[1].set_ylabel('Cycllone Frequency\n[$\mathregular{month^{-1}}$]',fontsize=12)
    axz[1].axhline(y=agetot_mean,color='k')
    #axz[1].axvline(x=2005,color='g')   
    # if period=='past':
    #     axz[1].set_ylim([70,95])
    axz[1].fill_between(index, agetot_mean, agetot.values.squeeze(), where=agetot.values.squeeze() >= agetot_mean, facecolor='red', interpolate=True,zorder=2)
    axz[1].fill_between(index, agetot_mean, agetot.values.squeeze(), where=agetot.values.squeeze() <= agetot_mean, facecolor='blue', interpolate=True,zorder=2)
    #axz[3].fill_between(index, agetot_mean, agetot_plus_1std, color='lightgrey', alpha=0.7)
    axz[1].fill_between(index, agetot_mean, agetot_plus_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    axz[1].fill_between(index, agetot_mean, agetot_min_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    # axz[4].fill_between(index, depth_mean, depth_plus_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    # axz[4].fill_between(index, depth_mean, depth_min_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    # axz[5].fill_between(index, slp_mean, slp_plus_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    # axz[5].fill_between(index, slp_mean, slp_min_2std, color='grey', alpha=0.5,edgecolor='none')
    # #axz[5].fill_between(index, slp_mean, slp_min_3std, color='lightgrey', alpha=0.1)
    # #axz[4].plot(index,prec.values)
    axz[2].set_ylabel('90th Percentile\nPrecipitation [mm]',fontsize=12)
    axz[2].axhline(y=prec_mean,color='k')
    #axz[2].axvline(x=2005,color='g')   
    #axz[4].set_ylim([18,21.5])
    axz[2].fill_between(index, prec_mean, prec.values.squeeze(), where=prec.values.squeeze() >= prec_mean, facecolor='red', interpolate=True,zorder=2)
    axz[2].fill_between(index, prec_mean, prec.values.squeeze(), where=prec.values.squeeze() <= prec_mean, facecolor='blue', interpolate=True,zorder=2)
    #axz[6].fill_between(index, prec_mean, prec_plus_1std, color='lightgrey', alpha=0.7)
    axz[2].fill_between(index, prec_mean, prec_plus_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    axz[2].fill_between(index, prec_mean, prec_min_2std, color='grey', alpha=0.5,edgecolor='none')
    #axz[6].fill_between(index, prec_mean, prec_min_3std, color='lightgrey', alpha=0.1)
    #axz[5].plot(index,ws.values)
    axz[3].set_ylabel('90th Percentile\nWind Speed [m/s]',fontsize=12)
    axz[3].axhline(y=ws_mean,color='k')
    #axz[3].axvline(x=2005,color='g')   
    axz[3].fill_between(index, ws_mean, ws.values.squeeze(), where=ws.values.squeeze() >= ws_mean, facecolor='red', interpolate=True,zorder=2)
    axz[3].fill_between(index, ws_mean, ws.values.squeeze(), where=ws.values.squeeze() <= ws_mean, facecolor='blue', interpolate=True,zorder=2)
    #axz[7].fill_between(index, ws_mean, ws_plus_1std, color='lightgrey', alpha=0.7)
    axz[3].fill_between(index, ws_mean, ws_plus_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    axz[3].fill_between(index, ws_mean, ws_min_2std, color='grey', alpha=0.5,edgecolor='none')
    ...
    fig.subplots_adjust(left=.07,right=.985,top=.95,bottom=.055,hspace=.075)
    if period=='past':
        fig.suptitle('30-year running averages temperature anomalies and cyclone features in the Mediterranean',fontsize=16)
    else:
    ...
    fig.savefig(path+'figs/Toulouse/Timeseries_{}_{}_alt.png'.format(track_loc,period),dpi=500)
    #plt.close(fig)
