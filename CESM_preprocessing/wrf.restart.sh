...
previous_date=$(date -d "${start_date:0:10} -${nd}days 00:00:00" +"%Y-%m-%d %H:%M:%S")
...
# check if we have reached the end of the full simulation. In case this is true, set simday to zero
end_date_sim_second=$(date -d "${end_date_sim:0:10} 00:00:00" +"%s")
end_date_second=$(date -d "${end_date:0:10} 00:00:00" +"%s")

if [[ ${end_date_sim_second} -ge ${end_date_second} ]]; then
   simday=0
...
#write the number of simulation days into a file
rm -rf simday.sh
cat>simday.sh<<EOF2
  #!/bin/bash
  export sday=$simday
...
set -ex
cd $SCRATCH/${exp_name}/jobs
# initiate new WPS chunk
./run_wps.sh ${start_date:0:10} ${end_date_sim:0:10} $exp_name $part
cd $SCRATCH/${exp_name}/jobs
...
sed -i 's/BOOLEAN/'true'/g'      namelist.input
...
kk=\`squeue -hn WRF-p${part}-${exp_name} -u ${user_name} -t Pending | awk '{print substr(\$0,1,8);exit}'\`
...
if [[ $simday != 0 ]]; then
  # initialize next chunk in case we have not reached the end of the simulation
  cd $SCRATCH/${exp_name}/jobs
  ./wrf.restart.sh $((part+1)) ${end_date_sim:0:10}
fi

cd $SCRATCH/${exp_name}/WRF/${start_date:0:10}
ln -sf $SCRATCH/${exp_name}/WRF/${previous_date:0:10}/wrfrst_d0*_${start_date:0:10}* .
...
if [ ! -d $SCRATCH/WRF_executables/${exp} ] || [ ! -f $SCRATCH/WRF_executables/${exp}/wrf.exe ] || [ ! -f $SCRATCH/WRF_executables/${exp}/real.exe ]; then
  ...
  . simday.sh
  . export_job.sh
  sbatch --dependency=afterok:$ff:$kk run_real.sh
else 
  . simday.sh
  . export_job.sh
  sbatch --dependency=afterok:$kk run_real.sh
# sbatch run_real.sh
fi
kkk=`squeue -hn real-p${part}-${exp_name} -u ${user_name} -t Pending | awk '{print substr($0,1,8);exit}'`
