...
#module load CDO/2.0.5-foss-2020a
#module load NCL/6.6.2-foss-2020a
#module load CDO
#module load NCL
#module load NCO
...
#                                                 SOILICE= Soil Ice
 ...
 P0_file="$scratchdir/P0.hyam.hybm.nc"
 if [[ ! -f "$P0_file" ]]; then
   ncks -v P0,hyam,hybm input.nc $P0_file
 fi
...
# Add liquid water and ice in each l*evel to get the total quantity of moisture for each month
...
# 
  ...
  for var in T M; do
    ...
    cdo -f grb settaxis,$new_year-01-01,00:00:00,6h LM-CESM.nc tmp.grb
    cdo setcode,25 tmp.grb LM-CESM.grb

    ### tbd check if topography is working ###
    cdo divc,9.81 -selvar,PHIS input.nc TER-CESM.nc
    cdo -f grb settaxis,$new_year-01-01,00:00:00,6h TER-CESM.nc tmp.grb
    cdo setcode,26 tmp.grb TER-CESM.grb

cdo merge MS1.grb MS2.grb MS3.grb MS4.grb PS.grb PSL.grb RH2.grb SST.grb T2.grb TS1.grb TS2.grb TS3.grb TS4.grb TS.grb U10.grb V10.grb CESM-${new_year}.2d.grib
cdo merge RH.grb T.grb U.grb V.grb Z.grb CESM-${new_year}.3d.grib
##rm -f *.nc *.grb lev*
